#ifndef FC_I2C_HEAD_H
#define FC_I2C_HEAD_H
...
{
Q_OBJECT
Q_DISABLE_COPY_MOVE(FCI2CHead)
public:
    const int HeadStateIntervalMs = 1000;
    QByteArray SecurityCode = {"fldskfks;lfk;sl"}; // просто набор символов
    ...
    explicit FCI2CHead(FCI2CBus *bus, uint8_t address = 0, QObject *parent = nullptr)
        : FCI2CDevice(bus, address, "head", parent)
    {
        init();
    }

    ~FCI2CHead() override = default;
