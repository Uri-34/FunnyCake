...
void FCSVGImageParser::simplifyPath(QVector<FC3DPoint> &points, qreal tolerance) noexcept
{
    ...
    while(!stack.isEmpty())
    {
        ...
        for(int i = first + 1; i < last; ++i)
        {
            ...
            {
                qreal t = qBound(qreal(0), ((points[i].x() - p1x) * dx + (points[i].y() - p1y) * dy) / lenSq, qreal(1));
...
FCSVGImageParser::ParseResult FCSVGImageParser::parse(const QString &filePath)
{
    ...
    {
        ParseResult res;
        res.errorMessage = QStringLiteral("не открыт файл: %1").arg(filePath);
        _state.set(FCErrorType::Parse);
        return res;
    }
...
void FCSVGImageParser::parseAsync(const QString &filePath)
{
    ...
    _state.set(FCReadyState::NotReady, FCPlayState::Start);
    ...
    _future = QtConcurrent::run([this, filePath]()
    {
        ...
        if(!file.open(QIODevice::ReadOnly | QIODevice::Text))
        {
            ...
            _state.set(FCPlayState::Stop, FCErrorType::Parse);
            emit error(QStringLiteral("не открыт файл: %1").arg(filePath));
            return;
        }
        ...
        _isParsing.storeRelaxed(0);
        _state.set(FCPlayState::Stop);

        if(res.success)
        {
            _state.set(FCReadyState::Ready);
...
FCSVGImageParser::ParseResult FCSVGImageParser::parseSvgInternal(const QByteArray &data)
{
    ...
    while(!reader.atEnd() && !reader.hasError())
    {
        ...
        if(reader.tokenType() == QXmlStreamReader::StartElement)
        {
            ...
            if (currentPercent > lastPercent) {
                emit progress(currentPercent, "парсятся элементы");
            ...
            FCImageBinaryContainer::BinaryLayer *curLayer = layerStack.isEmpty() ? &_container->defaultLayer() : layerStack.top();
        ...
        else if(reader.tokenType() == QXmlStreamReader::EndElement && reader.name() == QLatin1String("g"))
        {
            popTransform();
    ...
    if(reader.hasError())
    {
        result.errorMessage = QStringLiteral("XML error at line %1: %2").arg(reader.lineNumber()).arg(reader.errorString());
        delete _container;
        _container = nullptr;
        _state.set(FCErrorType::Parse);
        return result;
    ...
    emit progress(55, "оптимизация геометрии");
...
bool FCSVGImageParser::parseSvgElement(QXmlStreamReader &reader, FCImageBinaryContainer::BinaryLayer &layer)
{
    Q_UNUSED(layer)
...
{
    if(!_container || !_state.is(FCReadyState::Ready))
    {
        return false;
    }
    ...
    for(const auto &layer : _container->layers())
    {
        ...
        for(const auto &fig : layer.figures)
        {
            ...
            out << "; Points: " << fig.points.size() << (fig.isFilled ? " (filled)" : "") << "\n";
